import { r as reactExports, j as jsxRuntimeExports } from "./index-CioOM8pl.js";
...
function HeroAnimation() {
  ...
  reactExports.useEffect(() => {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      const color = SAFFRON_COLORS[Math.floor(Math.random() * SAFFRON_COLORS.length)];
      particles.push({
        x: Math.random() * width,
        ...
        vz: (Math.random() - 0.5) * 2e-3,
        radius: Math.random() * 2.5 + 0.5,
        color,
        alpha: Math.random() * 0.6 + 0.2,
        alphaSpeed: (Math.random() - 0.5) * 8e-3
      });
    }
    const makeRings = (w, h) => [
      {
    ...
    const drawRing = (ring, cx, cy) => {
      const bob = Math.sin(t * ring.bobSpeed + ring.bobOffset) * ring.bobAmplitude;
      const steps = 200;
      ctx.beginPath();
      for (let i = 0; i <= steps; i++) {
        const theta = i / steps * Math.PI * 2 + ring.angle;
        const px = cx + Math.cos(theta) * ring.radiusX;
        const py = cy + bob + Math.sin(theta) * ring.radiusY * Math.cos(ring.tilt);
